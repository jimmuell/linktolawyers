...
const FILTER_TABS: { label: string; value: 'accepted' | 'closed' | undefined }[] = [
...
export default function ClientCasesScreen() {
  ...
  const [statusFilter, setStatusFilter] = useState<'accepted' | 'closed' | undefined>(undefined);

  const { data: cases, isLoading, refetch, isRefetching } = useCases('client', statusFilter);

  const handleCasePress = useCallback(
    (caseData: CaseWithDetails) => {
      router.push(`/(client)/cases/${caseData.request.id}`);
    },
    [router],
  );
        ...
        <FlatList
          ...
          ListEmptyComponent={
            <EmptyState
              icon="work"
              title="No cases yet"
              description="When you accept a quote, it will appear here as an active case."
            />
          }
        />
      )}
